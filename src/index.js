...
if (
  !TOKEN &&
  (!process.env.SRE_ADMIN_EMAIL || !process.env.SRE_ADMIN_PASSWORD)
) {
  log(
    "error",
...
async function ensureToken() {
  ...
  if (!process.env.SRE_ADMIN_EMAIL || !process.env.SRE_ADMIN_PASSWORD) {
    throw new Error(
      "Authentication required: Set SRE_ADMIN_TOKEN or (SRE_ADMIN_EMAIL + SRE_ADMIN_PASSWORD)",
    );
  }

  log("info", "Authenticating with API", {
    email: process.env.SRE_ADMIN_EMAIL,
  });
...
server.tool(
  "get_dashboard",
  "Get business dashboard summary with key metrics.\n\nReturns: Total revenue, order counts (today/this week/this month), active customers, pending orders. Use for morning briefing or quick business health check.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  ...
  "List orders with optional status filter.\n\nReturns: Array of orders with customer info, totals, and current status. Use to view all orders or filter by status (pending, confirmed, processing, packed, shipped, out_for_delivery, delivered, cancelled).",
  {
    status: z
      ...
      .optional()
      .describe(
        "Filter: pending, confirmed, processing, packed, shipped, out_for_delivery, delivered, cancelled",
      ),
...
server.tool(
  ...
  async ({ order_id, status, notes }) => {
    await ensureToken();
    const data = await api("PATCH", `/admin/orders/${order_id}/status`, {
      status,
      notes,
    });
    return {
      content: [
        { type: "text", text: `Order ${data.order_number} → ${data.status}` },
      ],
    };
  },
);
...
server.tool(
  "list_products",
  "List all products with prices and stock.\n\nReturns: All products with SKU, retail/wholesale prices, and current stock quantities. Use to view catalog or check inventory levels.",
  {},
  async () => {
    await ensureToken();
  ...
  async (params) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Product created: ${data.name} (${data.sku}) — ₹${data.retail_price}`,
        },
      ],
    };
  },
  ...
  async ({ product_id, ...updates }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Updated ${data.name}: retail=${data.retail_price}, wholesale=${data.wholesale_price}, stock=${data.stock_quantity}`,
        },
      ],
    };
  },
...
server.tool(
  ...
  async ({ product_id, quantity }) => {
    ...
    const data = await api(
      "PATCH",
      `/admin/products/${product_id}/stock?quantity=${quantity}`,
    );
    return {
      content: [
        { type: "text", text: `Stock updated to ${data.stock_quantity}` },
      ],
    };
  },
);
...
server.tool(
  ...
  "List customers with optional search.\n\nReturns: Customer records with contact info, type (retail/wholesale), tier, and credit limits. Use to find customers or filter by type.",
  {
    search: z
      .string()
      .optional()
      .describe("Search by name, phone, or business"),
    customer_type: z
      .string()
      .optional()
      .describe("Filter: retail or wholesale"),
  },
...
server.tool(
  ...
  async ({ user_id, ...updates }) => {
    ...
    return {
      content: [{ type: "text", text: `Customer ${data.full_name} updated` }],
    };
  },
);
...
server.tool(
  "list_invoices",
  "List all invoices.\n\nReturns: All generated GST invoices with amounts, dates, and payment status. Use to review billing history.",
  {},
  async () => {
    await ensureToken();
  ...
  async ({ order_id, due_date }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Invoice ${data.invoice_number} generated — ₹${data.total_amount} (CGST: ₹${data.cgst_amount}, SGST: ₹${data.sgst_amount})`,
        },
      ],
    };
  },
...
server.tool(
  ...
  "Get all customers with outstanding credit balances.\n\nReturns: Customers with credit owed, amounts, and aging. Use to track receivables and collection priorities.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  ...
  "Record a payment received from a customer.\n\nReturns: Payment record with updated balance. Use to log UPI, cheque, or cash payments.",
  {
    ...
    reference_number: z
      .string()
      .optional()
      .describe("UPI UTR, cheque number, etc."),
    description: z.string().optional(),
  },
  async ({ user_id, amount, reference_number, description }) => {
    await ensureToken();
    const data = await api("POST", "/admin/ledger/payment", {
      user_id,
      amount,
      reference_number,
      description,
    ...
    return {
      content: [
        {
          type: "text",
          text: `Payment ₹${data.amount} recorded. New balance: ₹${data.running_balance}`,
        },
      ],
    };
  },
...
server.tool(
  "get_churn_summary",
  "Get customer retention summary — tier distribution, churn risks, volume drops.\n\nReturns: Customer tier counts, reorder gap alerts, volume drop alerts. Use to identify at-risk customers and churn trends.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  "get_reorder_gaps",
  "Get customers who are overdue for their next order (churn risk).\n\nReturns: Customers past expected reorder date with risk levels. Use to prioritize retention outreach.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  "get_volume_drops",
  "Get customers whose order volume dropped 20%+ recently.\n\nReturns: Customers with volume decrease and prior volumes. Use to identify troubled accounts.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  "classify_tiers",
  "Reclassify all customers into A/B/C tiers based on recent behavior.\n\nReturns: Counts of customers in each tier. Use to update customer segmentation monthly.",
  {},
  async () => {
    await ensureToken();
    ...
    return {
      content: [
        {
          type: "text",
          text: `Tiers updated: A=${data.counts.A}, B=${data.counts.B}, C=${data.counts.C}`,
        },
      ],
    };
  },
...
server.tool(
  "get_slab_pricing",
  "Get current slab pricing for all products.\n\nReturns: Volume-based pricing tiers for products. Use to quote wholesale customers.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  "update_stock_by_name",
  "Update stock for a product by name (e.g., 'Rajalakshmi Gold - 250g').\n\nReturns: Product with updated stock. Use for quick inventory adjustments without UUID.",
  {
    product_name: z
      .string()
      .describe(
        "Product name or partial match like 'Gold 250g' or 'Royal 1kg'",
      ),
    quantity: z.number().describe("New stock quantity"),
  },
  async ({ product_name, quantity }) => {
    ...
    const match = items.find(
      (p) =>
        ...
        `${p.name} ${p.weight_display}`.toLowerCase().includes(q),
    );
    if (!match)
      return {
        content: [
          {
            type: "text",
            text: `No product matching '${product_name}'. Available: ${items.map((p) => p.name).join(", ")}`,
          },
        ],
      };
    const data = await api(
      "PATCH",
      `/admin/products/${match.id}/stock?quantity=${quantity}`,
    );
    return {
      content: [
        {
          type: "text",
          text: `${match.name}: stock updated to ${data.stock_quantity}`,
        },
      ],
    };
  },
...
server.tool(
  ...
  async ({ product_name, retail_price }) => {
    ...
    if (!match)
      return {
        content: [
          { type: "text", text: `No product matching '${product_name}'` },
        ],
      };
    const data = await api("PATCH", `/admin/products/${match.id}`, {
      retail_price,
    });
    return {
      content: [
        {
          type: "text",
          text: `${data.name}: price updated to ₹${data.retail_price}`,
        },
      ],
    };
  },
...
server.tool(
  "get_stock_status",
  "Get stock levels for all products — shows what needs restocking.\n\nReturns: Products categorized as out, low, or in stock with priorities. Use for inventory planning.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  ...
  async ({ updates }) => {
    ...
    for (const { product_name, quantity } of updates) {
      ...
      const match = items.find((p) => p.name.toLowerCase().includes(q));
      if (!match) {
        results.push(`${product_name}: NOT FOUND`);
        continue;
      }
      try {
        await api(
        ...
        results.push(`${match.name}: FAILED — ${e.message}`);
      }
    }
    return {
      content: [
        { type: "text", text: `Stock updated:\n${results.join("\n")}` },
      ],
    };
  },
);
...
server.tool(
  "get_analytics_customers",
  "Get customer analytics — totals, top 10, by type/tier.\n\nReturns: Customer counts by segment, type/tier distribution. Use to understand customer composition.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  ...
  async ({ status_from, status_to, notes }) => {
    ...
    const orders = await api(
      "GET",
      `/admin/orders?status=${status_from}&per_page=100`,
    );
    const items = orders.items || [];
    if (items.length === 0)
      return {
        content: [
          { type: "text", text: `No orders with status '${status_from}'` },
        ],
      };
    ...
    for (const o of items) {
      try {
        await api("PATCH", `/admin/orders/${o.id}/status`, {
          status: status_to,
          notes: notes || `Bulk update: ${status_from} → ${status_to}`,
        });
        results.push(`${o.order_number}: OK`);
      } catch (e) {
        results.push(`${o.order_number}: FAILED — ${e.message}`);
      }
    }
    return {
      content: [
        {
          type: "text",
          text: `Updated ${results.filter((r) => r.includes("OK")).length}/${items.length} orders:\n${results.join("\n")}`,
        },
      ],
    };
  },
...
server.tool(
  "daily_summary",
  "Morning briefing — orders, revenue, pending dispatch, churn alerts, stock warnings in one call.\n\nReturns: Aggregated dashboard, delivery pipeline, customer health, and inventory alerts. Use for daily standup or overnight summary. Makes 5 parallel API calls.",
  {},
  async () => {
    await ensureToken();
    ...
    const summary = {
      revenue: {
        ...
        this_month: dashboard.orders_this_month + " orders",
      },
      orders: {
        today: dashboard.orders_today,
        this_week: dashboard.orders_this_week,
        pending: dashboard.pending_orders,
      },
      delivery: {
        pending_dispatch: Array.isArray(pending) ? pending.length : 0,
        in_transit: Array.isArray(active) ? active.length : 0,
      },
      customers: {
        active: dashboard.active_customers,
        churn_risks: churn.reorder_gap_alerts || 0,
        volume_drops: churn.volume_drop_alerts || 0,
      },
      stock_warnings: lowStock.map((p) => ({
        name: p.name,
...
server.tool(
  "customer_report",
  "Customer insights — top customers, new vs returning, at-risk.\n\nReturns: Total customer count, new customers (30 days), top 10 by revenue, type/tier distribution, churn risks. Use to identify key accounts and growth opportunities.",
  {},
  async () => {
    await ensureToken();
    ...
    for (const o of allOrders) {
      ...
      if (!ordersByCustomer[o.user_id])
        ordersByCustomer[o.user_id] = { count: 0, total: 0 };
    ...
    const topCustomers = custs
      .map((c) => ({
        ...
        tier: c.tier,
        orders: ordersByCustomer[c.id]?.count || 0,
        revenue: ordersByCustomer[c.id]?.total || 0,
      }))
          ...
          text: JSON.stringify(
            {
              ...
              by_type: {
                retail: custs.filter((c) => c.customer_type === "retail")
                  .length,
                wholesale: custs.filter((c) => c.customer_type === "wholesale")
                  .length,
              },
              tier_distribution: churn.tier_distribution || {},
              churn_risks: churn.reorder_gap_alerts || 0,
...
server.tool(
  ...
  "Calculate profit margin given buy and sell prices.\n\nReturns: Profit/loss with margin %, GST amount, per-kg breakdown. Use for pricing decisions.",
  {
    ...
    packaging_cost_per_kg: z
      .number()
      .default(0)
      .describe("Packaging cost per kg"),
    delivery_cost_per_kg: z
      .number()
      .default(0)
      .describe("Delivery cost per kg"),
  },
  async ({
    buy_price_per_kg,
    sell_price_per_kg,
    ...
    const totalCost =
      (buy_price_per_kg + packaging_cost_per_kg + delivery_cost_per_kg) *
      volume_kg;
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { count: summary.length, orders: summary },
            null,
            2,
          ),
        },
...
server.tool(
  "restock_alert",
  "Check which products are low on stock and need restocking.\n\nReturns: Out-of-stock products, low-stock items with thresholds, and healthy inventory. Use for procurement planning.",
  {},
  async () => {
    await ensureToken();
    const data = await api("GET", "/products?per_page=50");
...
server.tool(
  ...
  async ({ customers }) => {
    await ensureToken();
    const data = await api("POST", "/admin/customers/bulk-upload", {
      customers,
    });
    return {
      content: [
        {
          type: "text",
          text: `Bulk upload: ${data.created} created, ${data.skipped} skipped (duplicate), ${data.errors?.length || 0} errors`,
        },
      ],
    };
  },
...
server.tool(
  ...
  "Get orders ready to be dispatched",
  {},
  async () => {
    await ensureToken();
    ...
    if (!data?.length)
      return {
        content: [{ type: "text", text: "No orders pending dispatch" }],
      };
    const lines = data.map(
      (o) =>
        `${o.order_number} | ₹${o.total_amount} | ${o.shipping_address?.city || "—"} | ${o.status}`,
    );
    return {
      content: [
        {
          type: "text",
          text: `${data.length} orders pending dispatch:\n${lines.join("\n")}`,
        },
      ],
    };
  },
...
server.tool(
  ...
  async () => {
    ...
    if (!data?.length)
      return { content: [{ type: "text", text: "No active deliveries" }] };
    const lines = data.map(
      (o) =>
        `${o.order_number} | ₹${o.total_amount} | ${o.shipping_address?.city || "—"} | ${o.status}`,
    );
    return {
      content: [
        {
          type: "text",
          text: `${data.length} active deliveries:\n${lines.join("\n")}`,
        },
      ],
    };
  },
...
server.tool(
  "get_tea_stock",
  "Get raw tea stock levels — total kg by tea type in warehouse.\n\nReturns: Tea types with quantities, last purchase info, and purchase prices. Use for raw material planning.",
  {},
  async () => {
    await ensureToken();
    ...
    if (!data?.length)
      return {
        content: [{ type: "text", text: "No tea types in stock tracking yet" }],
      };
    const lines = data.map(
      (s) =>
        `${s.tea_type}: ${s.total_kg} kg (last buy: ${s.last_purchase_kg}kg @ ₹${s.purchase_price_per_kg}/kg)`,
    );
    return {
      content: [{ type: "text", text: `Raw Tea Stock:\n${lines.join("\n")}` }],
    };
  },
);
...
server.tool(
  "create_tea_stock",
  "Add a new tea type to warehouse stock tracking.\n\nReturns: New tea stock record with UUID. Use to start tracking new tea types.",
  {
    tea_type: z
      .string()
      .describe("Name of the tea type (e.g., 'Nilgiri Dust', 'Assam CTC')"),
    total_kg: z.number().default(0).describe("Initial stock in kg"),
    purchase_price_per_kg: z
      .number()
      .default(0)
      .describe("Purchase price per kg"),
  },
  async ({ tea_type, total_kg, purchase_price_per_kg }) => {
    await ensureToken();
    const data = await api("POST", "/admin/analytics/tea-stock", {
      tea_type,
      total_kg,
    ...
    return {
      content: [
        {
          type: "text",
          text: `Tea type '${data.tea_type}' added with ${data.total_kg} kg`,
        },
      ],
    };
  },
...
server.tool(
  ...
  async ({ stock_id, add_kg, reduce_kg, total_kg, purchase_price_per_kg }) => {
    ...
    if (purchase_price_per_kg !== undefined)
      body.purchase_price_per_kg = purchase_price_per_kg;
    const data = await api(
      "PATCH",
      `/admin/analytics/tea-stock/${stock_id}`,
      body,
    );
    return {
      content: [
        { type: "text", text: `${data.tea_type}: now ${data.total_kg} kg` },
      ],
    };
  },
);
...
server.tool(
  "list_bundles",
  "List all product bundles.\n\nReturns: All bundles with items, pricing, and active state. Use to review bundle offerings.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  "update_bundle",
  "Update a bundle's pricing, items, or active state.",
  {
    bundle_id: z.string(),
    name: z.string(),
    slug: z.string(),
    items: z.array(
      z.object({ product_id: z.string(), quantity: z.number().int() }),
    ),
    bundle_price: z.number().nullable().optional(),
...
server.tool(
  "list_flash_sales",
  "List all flash sales (scheduled + live + ended).",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  "create_flash_sale",
  "Create a timed discount. Empty product_ids = site-wide.\n\nReturns: Sale UUID. Use to run limited-time promos.",
  {
    title: z.string(),
  ...
  async (body) => {
    ...
    return {
      content: [{ type: "text", text: `Flash sale created: ${data.id}` }],
    };
  },
);
...
server.tool(
  "update_flash_sale",
  "Update flash sale timing, discount, or status.",
  {
    sale_id: z.string(),
    title: z.string(),
    banner_text: z.string().optional(),
...
server.tool(
  ...
  async ({ include_depleted }) => {
    ...
    const data = await api(
      "GET",
      `/admin/batches?include_depleted=${include_depleted}`,
    );
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
  },
);
...
server.tool(
  ...
  "Batches expiring within N days (default 90). Use to flag flash-sale candidates.",
  {
  ...
  async ({ within_days }) => {
    ...
    const data = await api(
      "GET",
      `/admin/batches/expiring?within_days=${within_days}`,
    );
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
  },
);
...
server.tool(
  ...
  "Record a new tea batch received from supplier.\n\nReturns: Batch UUID. Use for lot traceability + expiry tracking.",
  {
    ...
    tea_type: z
      .string()
      .optional()
      .describe("e.g. Premium Blend / Normal Blend"),
...
server.tool(
  "get_ticket",
  "Get full ticket thread with all replies.",
  {
    ticket_id: z.string(),
  },
  async ({ ticket_id }) => {
...
server.tool(
  "reply_to_ticket",
  "Reply to a customer support ticket. Auto-emails customer.",
  {
    ticket_id: z.string(),
    body: z.string().min(1),
  },
...
server.tool(
  "list_campaigns",
  "List all email newsletter campaigns with send stats.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  "preview_newsletter_recipients",
  "Count recipients for a newsletter segment before sending.",
  {
    subject: z.string(),
    body_html: z.string(),
    segment: z
  ...
  async (body) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `${data.count} recipients for segment '${data.segment}'`,
        },
      ],
    };
  },
  ...
  async (body) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Sent to ${result.sent}/${result.recipients} (${result.errors} errors)`,
        },
      ],
    };
  },
...
server.tool(
  "gstr1_csv_url",
  "Get GSTR-1 invoice-wise CSV URL for a month. Download from admin panel UI.",
  {
    year: z.number().int(),
  ...
  async ({ year, month }) => {
    ...
    return {
      content: [
        { type: "text", text: `CSV download: ${url} (requires auth header)` },
      ],
    };
  },
);
...
server.tool(
  "ar_aging",
  "Accounts receivable aging — customers with outstanding credit grouped by age bucket (0-30, 31-60, 61-90, 90+).\n\nReturns: Per-customer + grand total. Use to prioritize collections.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  ...
  "List carts idle > 1 hour with items. Use to target recovery campaigns.",
  {},
  async () => {
    await ensureToken();
...
server.tool(
  "nudge_abandoned_cart",
  "Send abandoned cart reminder (WhatsApp + SMS fallback) to one user.",
  {
    user_id: z.string(),
  },
  async ({ user_id }) => {
...
server.tool(
  "auto_nudge_all_carts",
  "Bulk-nudge every qualifying abandoned cart (>1h idle, not nudged in last 24h).",
  {},
  async () => {
    await ensureToken();
    ...
    return {
      content: [
        {
          type: "text",
          text: `Nudged ${data.nudged} users (${data.errors?.length || 0} errors)`,
        },
      ],
    };
  },
...
server.tool(
  ...
  "View a customer's loyalty balance + history.",
  {
    user_id: z.string(),
  },
  async ({ user_id }) => {
...
server.tool(
  ...
  async ({ user_id, points, reason }) => {
    await ensureToken();
    const data = await api("POST", `/admin/loyalty/${user_id}/adjust`, {
      points,
      reason,
    });
    return {
      content: [
        {
          type: "text",
          text: `${points > 0 ? "+" : ""}${points} pts. New balance: ${data.new_balance}`,
        },
      ],
    };
  },
...
server.tool(
  "list_reviews",
  "List all product reviews for moderation. Filter by approved state.",
  {
...
server.tool(
  "moderate_review",
  "Approve/unapprove a review or attach admin reply.",
  {
    review_id: z.string(),
    approved: z.boolean().optional(),
...
server.tool(
  ...
  "Attach IRN (Invoice Reference Number) from GSTN IRP after manual registration.",
  {
    invoice_id: z.string(),
    irn: z.string(),
